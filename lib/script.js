...
if (largeHeadingBlocks) {
  const hexagonPath =
    "M 10 110 C 5 110 0.5 105.5 0.5 100.5 V 40 C 0.5 36.75 2 34 4.5 32 L 47.75 2.5 C 49 1.5 51 1 53 1 H 100 C 105 1 109.5 5.5 109.5 10.5 V 69.75 C 109.5 72.75 108 75.5 105.5 77.5 L 62 108 C 60.75 109 59 110 57 110 H 10 Z";
  ...
  const squarePath =
    "M 10 110 C 5 110 0.5 105.5 0.5 100.5 V 40 C 0.5 36.75 0.5 35.25 0.5 32 L 0.5 10.5 C 0.5 5.5 5 1 10 1 H 100 C 105 1 109.5 5.5 109.5 10.5 V 69.75 C 109.5 72.75 109.5 74.5 109.5 77.5 L 110 100.5 C 110 105.5 105 110 100 110 H 10 Z";
  ...
  mainNavLinks.forEach((link) => {
    link.addEventListener("click", (evt) => {
      ...
      const activeContent = header.querySelector(
        ".navContent [content-status='active']"
      );
      if (activeTab) {
      ...
      if (
        activeContent &&
        target !== activeContent.getAttribute("data-target")
      ) {
      ...
      const setActive = header.querySelector(
        `.navContent [data-target='${target}']`
      );
      setTimeout(() => {
...
const allHeaderEls = document.querySelectorAll(
  "header a, header .searchWrapper"
);
if (allHeaderEls) {
...
const aboutSlides = document.querySelectorAll(
  ".history_section .heading_block .swiper-slide"
)?.length;
...
if (
  historySection &&
  aboutSwiperDiv &&
  parseInt(aboutSlides) - 2 === yearBlocks.length
) {
